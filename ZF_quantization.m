...
%         tic
    for j = 1:length(snr)
        ...
        for d = 1:N_d
            H_hat(:, d, :) = cscb_est(rx_H(:,d,:), b_cb, h_cb, 'n', No);
        end
        ...
        % precoding 수행
        [Dsym, factor, Wd] = ZF_precoding(Dsym, H_hat);
        ...
        for k = 1:fft_len
            ...
            % 자기 채널
            H_A(:,k) = Wr.' * tmp__ * t_Wd / factor(k) .* eye(N_s) * ones(N_s,1);
            
            % 간섭 채널
            H_I(:,k) = Wr.' * tmp__ * t_Wd / factor(k) .* (ones(N_s) - eye(N_s)) * ones(N_s,1);
            n = Wr.' * awgn_noise(zeros(N_rx*N_d,1), snr(j));
            H_I(:,k) = H_I(:,k) + n;
        end
        
        % 최종 sum rate 계산
        res_temp = mean( sum( log2( ones(N_d,1) + abs(H_A).^2 ./ abs(H_I).^2 ) ) );
        if isnan(res_temp)
            temp_num = temp_num - 1;
            continue;
