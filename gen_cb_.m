function [cb, sample, i_sample] = gen_cb_(a, n_bit, ang, num)
...
% 변수 초기화
if nargin < 4, num = 5000; end
if nargin < 3, ang = 2*pi; end
iter  = 1000;
[dim, ~] = size(a);
if sum(imag(a)) == 0, mode = 'r'; else mode = 'c'; end
M = 2^n_bit;    % 양자화 상태의 수
Q = M * num;    % 코드북이 대표하는 샘플들의 수
if mode == 'r', i_sample = rand(Q,dim);
elseif mode == 'c', i_sample = 0.5 * (rand(Q,dim) + 1j*rand(Q,dim));
end

% 주어진 기저벡터와 유사도가 높은 벡터를 먼저 선별 후 코드북 후보 초기화
sample = i_sample;
idx = find( ang < acos( real( nom(sample) * nom(a.')' ) ) );
sample(idx,:) = [];
[Q, ~] = size(sample);
cb = sample(1:M,:);
